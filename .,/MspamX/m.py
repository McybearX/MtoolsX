...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
def ting():
	sistem("play-audio ../../.sound/ting.wav")
...
def menu():
	...
	emil(u+" ["+m+"𝚡"+u+"] "+m+"𝚈𝚘𝚞𝚝𝚞𝚋𝚎"+p+" : MBEWLEGS")
...
def tokped():
         ...
         dela = json.loads(rek)
         if "Maaf, Permohonan Anda tidak dapat diproses saat ini. Mohon dicoba kembali." in dela['error_message']:emil(u+" ["+m+"x"+u+"]"+k+" Tokped Limiit...")
         elif "Anda sudah melakukan 3 kali pengiriman kode" in dela["error_message"]:emil(u+" ["+m+"x"+u+"]"+k+" Tokped Limit...")
         elif " " in dela['error_message']:emil(dela);emil(u+" ["+h+"x"+u+"]"+p+" Tokped Sukses")
         else:emil(u+" ["+h+"x"+u+"]"+p+" tokped Suksess")
def call_bts():
	...
	if "Anda akan menerima sebuah panggilan dari sistem kami. Silakan isi 6 ANGKA TERAKHIR dari nomor telepon dibawah ini." in _usupp_:emil(u+" ["+h+"x"+u+"]"+p+" Call Bts Sukses")
	else:emil(u+" ["+m+"x"+u+"]"+k+" Call Bts Limit...")
def cunuy():
	req=requests.get("https://ainxbot-sms.herokuapp.com/api/spamsms",params={"phone":nohap}).text
	if "terkirim" in req:		emil(u+" ["+h+"x"+u+"]"+p+" Bonus BOM Sms Terkirim");time.sleep(3);		emil(u+" ["+h+"x"+u+"]"+p+" + 2× Call Greater Jakarta")
	elif "Internal Server Error" in req:		emil(u+"["+m+"x"+u+"]"+k+" Limit...")
	else:		emil(req);		emil(u+"["+m+"x"+u+"]"+k+" Eror...")
def nutriclub():
	nom="0"+nohap
...
def OYO():
	...
	if y1 == True:
		emil(u+" ["+h+"x"+u+"]"+p+" OYO Sukses ")
	else:
		emil(u+" ["+m+"x"+u+"]"+k+" OYO Limit..")

if Bismillahirohmanirrohim == "Aaammiin":
#	sistem("xdg-open https://cararegistrasi.com/vxeN9T0zKha")
	menu()
